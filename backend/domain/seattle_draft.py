...
from fastapi_backend.domain import Player, Team, TeamDraft, TeamOptimizationParameters
from fastapi_backend.draft import TeamDraft, SeattleDraft
...
def optimize_seattle_selection_scenario(
    ...
    # Objective
    perf_obj = pulp.lpSum(
        [player.metrics[perf_metric] for player in existing_teams_draft.exposed_players if select_var[player.id]]
    )
    fin_obj = pulp.lpSum(
        [player.contract[fin_metric] for player in existing_teams_draft.exposed_players if select_var[player.id]]
    )
    age_obj = pulp.lpSum(
        [player.age for player in existing_teams_draft.exposed_players if select_var[player.id]]
    )
    ...
    [select_var[player.id] for player in existing_teams_draft.exposed_players if player.forward]
    )
    model += select_F_constraint >= 14, "SelectForward"

    select_D_constraint = pulp.lpSum(
    [select_var[player.id] for player in existing_teams_draft.exposed_players if player.defence]
    )
    model += select_D_constraint >= 9, "SelectDefence"

    select_G_constraint = pulp.lpSum(
    [select_var[player.id] for player in existing_teams_draft.exposed_players if player.goalie]
    )
    ...
    [select_var[player.id] for player in existing_teams_draft.exposed_players if ((player.forward or player.defence) and player.under_ct)]
    )
    model += select_skater_contract_constraint >= 18, "SelectContractSkater"

    select_G_contract_constraint = pulp.lpSum(
    [select_var[player.id] for player in existing_teams_draft.exposed_players if (player.goalie and player.under_ct)]
    )
    ...
    [player.contract["cap_hit_20_21"] for player in existing_teams_draft.exposed_players if select_var[player.id]]
    )
    ...
    [select_var[player.id] for player in existing_teams_draft.exposed_players if player.center]
    )
    model += select_C_constraint >= 4, "SelectC"

    select_LW_constraint = pulp.lpSum(
    [select_var[player.id] for player in existing_teams_draft.exposed_players if player.left_wing]
    )
    model += select_LW_constraint >= 4, "SelectLW"

    select_RW_constraint = pulp.lpSum(
    [select_var[player.id] for player in existing_teams_draft.exposed_players if player.right_wing]
    )
...
def get_seattle_draft_decisions(
    params: TeamOptimizationParameters,
    existing_teams_draft: TeamDraft
) -> SeattleTeamDraft:
    """Returns seattle decisions under the given optimization parameters."""

    def model_results_to_seattle_draft(model: pulp.LpProblem, existing_teams_draft:TeamDraft) -> SeattleDraft:
    ...
    seattle_selection_model = optimize_seattle_selection_scenario(params, existing_teams_draft)
